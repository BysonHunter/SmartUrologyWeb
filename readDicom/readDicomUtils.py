...
def readDicomFolder(dicom_path, images_path):
    ...
    get_images_from_slice(pixel_array, save_dir, slices, frame_Size)
...
def read_dicom_set(dicom_path):
    ...
    if dicom_current_dir_path is not None:
        slices = readDICOMDIR(dicom_current_dir_path)
    else:
        slices = readDicomFiles(dicom_path)
...
def get_images_from_slice(data_array, images_path, slices, frameSize):
    ...
    with open(dir_info, 'w') as f:
        ...
        for y in range(start_pos, stop_pos):
            ...
            # coronal_slice = setDicomWinWidthWinCenter(coronal_slice, 400, 40,
            # slices[0].Rows, slices[0].Columns)
